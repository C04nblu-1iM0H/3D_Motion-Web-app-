...
export default function Menu() {
    ...
    const routeCourse = pathname === '/adminpanel/course' || pathname === '/adminpanel/course/create_course' || pathname === '/adminpanel/course/view_courses';
    useState(() => {
        if (isActiveInList) {
            setIsOpen(true);
        }
                    ...
                    <ul className={`pl-5 duration-700 ease-in-out ${isOpen ? 'h-20 opacity-100' : 'h-0 opacity-0 overflow-hidden'}`}>
                        ...
                        <li>
                            <Link 
                                ...
                                onClick={() => dispatch(setSelectedPanel('googletable'))}
                            >
                                <BsDashLg />
